...
根路径 `/` 会跳转到 `/chat/`。这个页面使用固定的同源 `/v1/demo/...`
公开代理，覆盖角色目录、连续会话、流式与非流式对话、RAG 检索来源、长期记忆查询与
删除，以及常用生成参数。应用会把这些请求映射到对应的 `/v1/...` 接口，并强制
`user_id` 使用随机生成的 `demo-` 作用域；反向代理仍应只允许这些安全路由，在
服务端注入绑定应用的服务令牌并实施公网限流。不得把服务令牌写入 HTML、JavaScript
或浏览器存储。语料导入、令牌与后台管理接口不属于公开演示范围。
