"""Model provider registry configuration parsing."""
...
class ModelProviderConfig:
    ...
    chatCompletionsPath: str | None = None
    ...
    def from_mapping(
        ...
        *,
        ...
        api_key_env = _optional_string(data.get("api_key_env", data.get("apiKeyEnv")))
        configured_api_key = _optional_string(data.get("api_key", data.get("apiKey")))
        return cls(
            provider_id=_require_non_empty(provider_id, "provider id"),
            provider_type=provider_type,
            baseUrl=_optional_string(data.get("base_url", data.get("baseUrl"))),
            timeoutMs=_int_value(
                data.get("timeout_ms", data.get("timeoutMs", 60000)),
                f"timeout_ms for {provider_id}",
            ),
            apiKey=configured_api_key or _api_key_from_env(api_key_env, env),
            apiKeyEnv=api_key_env,
            providerName=_optional_string(
                data.get("provider_name", data.get("providerName"))
            ),
            chatCompletionsPath=_optional_string(
                data.get(
                    "chat_completions_path",
                    data.get("chatCompletionsPath"),
                )
            ),
        )
...
class ModelProviderSettings:
    ...
    sourceEnv: Mapping[str, str] = field(default_factory=dict, repr=False)
    ...
    def from_legacy_mapping(cls, data: Mapping[str, str]) -> "ModelProviderSettings":
        ...
        api_key_env = data.get("MODEL_API_KEY_ENV")
        provider_config = ModelProviderConfig(
            ...
            apiKey=data.get("MODEL_API_KEY") or _api_key_from_env(api_key_env, data),
            apiKeyEnv=api_key_env,
