...
REVIEWED_PIPELINE_VERSION = f"{PIPELINE_VERSION}+agent-review.v1"
...
def build_reviewed_dialogue_records(
    ...
    *,
    ...
    for values in section_speech.values():
        values.sort(key=lambda item: (int(item[0].source["line"]), item[1].start))

    records: list[CorpusRecord] = []
    for section_key, speech in section_speech.items():
        del section_key
        for position, (unit, span, reviewed) in enumerate(speech):
            annotation = reviewed.annotation
            if (
                ...
                continue
            context = speech[max(0, position - 2) : position + 3]
            lines: list[str] = []
            context_speakers: list[str] = []
            for context_unit, context_span, context_reviewed in context:
                context_annotation = context_reviewed.annotation
                label = _speaker_label(context_annotation.speaker)
                quote = _sanitize_nested_quotes(
                    context_unit,
                    context_span,
                    reviewed_spans,
                )
                lines.append(f"{label}：{converter(quote)}")
                if context_annotation.speaker not in context_speakers:
                    context_speakers.append(context_annotation.speaker)
            ...
            content = (header + "\n" + "\n".join(lines))[:MAX_RECORD_CONTENT_CHARS].strip()
            digest = hashlib.blake2b(span.span_id.encode("utf-8"), digest_size=10).hexdigest()
            document_id = (
            ...
            metadata: dict[str, Any] = {
                ...
                "review_method": (
                    "sol_adjudication" if "sol" in reviewed.model else "luna_first_pass"
                ),
